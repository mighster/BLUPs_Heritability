...
#Import new df
df<-read.csv("C:/Users/falk/Google Drive/PhD/PhD Projects/Blue Steel/Paper#2/Root Angles/Root Angle Tally.csv", header = T, check.names=FALSE)
...
Day6DataOutput <- data.frame(matrix(vector(),292,1, dimnames=list(c(), c("Name"))))

#fill empty dataframe with 1-292 so that the cbind will work later on
Day6DataOutput$Row <- c(1:292)
...
str(colnum)
for (i in 1:45){  #this second loop runs through each TRAIT, one at a time
...
#output that beast
write.csv(Day6DataOutput,"C:/Users/falk/Google Drive/PhD/PhD Projects/Blue Steel/Paper#2/Root Angles/BLUPsDay6_LBA.csv")
write.csv(Day6VarCompOutput,"C:/Users/falk/Google Drive/PhD/PhD Projects/Blue Steel/Paper#2/Root Angles/VarianceComponents_Day6_LBA.csv")
